...
//---------------------------------------
    ...
    path_container.push_back( strt_ptr->data );
         ...
         strt_ptr->right == nullptr     )
    {
        ...
        for (int i = 0; i < static_cast<int>( path_container.size() ); i++)
        {
            // Print The Node in the path:
            cout << path_container.at( i );
            
            // Print the path arrow:
            if( i != static_cast<int>( path_container.size() ) - 1 ){
               cout << "   ->   ";
            }
        }
...
//  has tree that was created correctly outputs the preOrder paths
